...
class InstaBot:
    ...
    def get_media_id_by_tag(self, tag):
        ...
        if (self.login_status):
            ...
            if self.login_status == 1:
                ...
                return 0
    def get_media_id_by_location(self, location):
        """ Get media ID set, by your location """
        if (self.login_status):
            log_string = "Get media id by location: %s" % (location)
            self.write_log(log_string)
            if self.login_status == 1:
                ...
                try:
                    ...
                    self.media_by_location = []
                    self.write_log("Except on get_media_location!")
            else:
                return 0
            ...
            if len(self.media_by_tag) != 0:
                ...
                for d in self.media_by_tag:
                    ...
                    if media_size > 0 or media_size < 0:
                        ...
                        return False
            elif len(self.media_by_location) != 0:
                i = 0
                for d in self.media_by_location:
                    # Media count by this tag.
                    if media_size > 0 or media_size < 0:
                        media_size -= 1
                        l_c = self.media_by_location[i]['likes']['count']
                        if ((l_c <= self.media_max_like and
                             l_c >= self.media_min_like) or
                            (self.media_max_like == 0 and
                             l_c >= self.media_min_like) or
                            ...
                            for blacklisted_user_name, blacklisted_user_id in self.user_blacklist.items(
                                ...
                                if self.media_by_location[i]['owner'][
                                        'id'] == blacklisted_user_id:
                                    self.write_log(
                                        "Not liking media owned by blacklisted user: "
                                        + blacklisted_user_name)
                            ...
                            try:
                                caption = self.media_by_location[i][
                                    'caption'].encode(
                                        'ascii', errors='ignore')
                                tag_blacklist = set(self.tag_blacklist)
                                if sys.version_info[0] == 3:
                            ...
                            log_string = "Trying to like media: %s" % \
                                         (self.media_by_location[i]['id'])
                            self.write_log(log_string)
                            like = self.like(self.media_by_location[i]['id'])
                            # comment = self.comment(self.media_by_tag[i]['id'], 'Cool!')
                            # follow = self.follow(self.media_by_tag[i]["owner"]["id"])
                            if like != 0:
                                if like.status_code == 200:
    ...
    def new_auto_mod_location(self):
        while True:
            # ------------------- Get media_id -------------------
            if len(self.media_by_location) == 0:
                ...
                self.max_tag_like_count = random.randint(
                    1, self.max_like_for_one_tag)
            # ------------------- Like -------------------
            self.new_auto_mod_like_location()
            # ------------------- Follow -------------------
            self.new_auto_mod_follow()
            # ------------------- Unfollow -------------------
            self.new_auto_mod_unfollow()
    ...
    def new_auto_mod_like_location(self):
        if time.time() > self.next_iteration["Like"] and self.like_per_day != 0 \
            ...
            if self.like_all_exist_media(media_size=1, delay=False):
                ...
                self.this_tag_like_count += 1
                if self.this_tag_like_count >= self.max_tag_like_count:
                    self.media_by_location = [0]
            # Del first media_id
            del self.media_by_location[0]
    ...
    def new_auto_mod_follow(self):
        ...
        elif time.time() > self.next_iteration["Follow"] and \
